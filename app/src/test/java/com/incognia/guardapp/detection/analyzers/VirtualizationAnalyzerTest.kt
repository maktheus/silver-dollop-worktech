 ...
 * st = "0A" means TCP_LISTEN.  Port 27042 = 0x699A.
 */
class VirtualizationAnalyzerTest {
    ...
    fun `detects Frida listening on loopback port 27042`() {
        val signals = analyzer.parseTcpTable(listOf(row("0100007F:699A")))
    ...
    fun `detects Frida bound to wildcard address 0_0_0_0`() {
        val signals = analyzer.parseTcpTable(listOf(row("00000000:699A")))
        assertEquals(1, signals.size)
    }
    ...
    fun `multiple listening Frida sockets each produce a signal`() {
        val signals = analyzer.parseTcpTable(
            listOf(row("0100007F:699A"), row("00000000:699A"))
        )
    ...
    @Test
    fun `does not flag when 699A appears only in remote address`() {
        val signals = analyzer.parseTcpTable(
            listOf(row(local = "0200007F:C350", remote = "0100007F:699A", state = "01"))
        )
    ...
    fun `does not flag ESTABLISHED socket on Frida port`() {
        ...
        val signals = analyzer.parseTcpTable(listOf(row("0100007F:699A", state = "01")))
        assertTrue(signals.isEmpty())
    }
    ...
    fun `sourcePath is included in signal description when provided`() {
        val signal = analyzer.parseTcpTable(listOf(row("0100007F:699A")), "/proc/net/tcp")
        assertTrue(signal[0].description.contains("/proc/net/tcp"))
    }

    @Test
    fun `no sourcePath leaves description without per-path suffix`() {
        val signal = analyzer.parseTcpTable(listOf(row("0100007F:699A")))
